    ...
    samples.each {
        bat(
            ...
            script: """
                ...
                @echo on
                cd C:\\oe\\${oe_package}\\openenclave\\share\\openenclave\\samples\\${it}
                mkdir build
                cd build
            """
        )
        dir("C:\\oe\\${oe_package}\\openenclave\\share\\openenclave\\samples\\${it}\\build") {
            ninjaBuildCommand(cmakeArgs, "C:\\oe\\${oe_package}\\openenclave\\share\\openenclave\\samples\\${it}")
            bat(
                script: """
                    call vcvars64.bat x64
                    @echo on
